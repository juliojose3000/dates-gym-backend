...
class ReservationControllerTest {
	...
	private String url = "http://localhost:8080/gymcachi/reservation";
	...
	public static String getToken() {
		...
		try {
			mResponse = 	
					restTemplate.postForObject("http://localhost:8080/gymcachi/authenticate", auth, MyResponse.class);
		}catch(Exception e) {
			System.out.println(e.getMessage());
		}

		System.out.println(mResponse.getToken());
		
		return mResponse.getToken();
	...
	void cancel() {
		...
		HttpEntity<Reservation> request = new HttpEntity<Reservation>(getReservation(), headers);
		ResponseEntity<MyResponse> response = restTemplate.exchange(this.url+"/cancel", HttpMethod.POST, request, MyResponse.class);
		assertTrue(response.getStatusCode() == HttpStatus.OK);
		MyResponse mResponse = response.getBody();
